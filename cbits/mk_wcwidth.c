...
// allocated with init_custom_table and initialized with
// set_custom_table_range.
...
static int bisearch(HsChar ucs, const struct interval *table, int max) {
  int min = 0;
  int mid;
...
// table is marked ready, it will not be used by mk_wcwidth. To mark
// the table as ready, call activate_custom_table() after the table
// has been set up with calls to set_custom_table_range.
int init_custom_table(int size)
{
...
int set_custom_table_range(uint32_t start, uint32_t size, uint8_t width)
{
    if ((custom_table == NULL) ||
...
//
// After this call, further calls to set_custom_table_range will
// fail.
//
